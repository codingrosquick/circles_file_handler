...
# NOTE: this other function could be removed as well!
async def async_command_shell(command, verbose: bool = False):
    ...
    if verbose:
        # SEE THE COMMAND
        print('\n\ncommand', command, '\n\n')
        # FIND WHERE WE ARE DURING THE CALL
        where = subprocess.run(['pwd'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
...
#             print('remote_object_name', remote_object_name, '\n')
...
def findall_files_legacy(root, verbose: bool = False):
    '''
    finds all files within the root directory and recursively below
    :param root: str, root file from which to begin the search
    :param verbose: bool, set to True to see fuller logs
